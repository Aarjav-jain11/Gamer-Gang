        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50">
              {/* overlay: use solid semi-transparent background as fallback (no backdrop-filter) */}
              <div onClick={() => setMenuOpen(false)} className="absolute inset-0 bg-black/60 z-50" />
              <motion.div
                initial={{ x: '100%' }}
                animate={{ x: 0 }}
                exit={{ x: '100%' }}
                transition={{ type: 'spring', stiffness: 300, damping: 30 }}
                className="absolute right-0 top-0 h-full w-80 bg-[#07060a] p-6 shadow-2xl z-70"
                style={{
                  backdropFilter: 'none',
